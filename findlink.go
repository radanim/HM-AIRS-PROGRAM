...
func main() {
//
   link := flag.String("links", "","linksfile.json")
   target := flag.String("target", "","target geometry")
   var pX float64
   var pY float64
   ...
   file, _ := ioutil.ReadFile(*link)

    s := strings.Split(*target,",")
   x1, y1 := s[0], s[1]            
   

   pX, _ = strconv.ParseFloat(x1,64)   
   pY, _ = strconv.ParseFloat(y1,64)   
//
   data := Collection{}
